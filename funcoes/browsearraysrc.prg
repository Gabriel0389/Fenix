...
function BrowseArray(aArray)
      ...
      if !(HB_IsNil(aArray)) .and. HB_IsArray(aArray) .or. HB_ISHASH(aArray) .and. Len(aArray) != 0         
         ...
         for EACH a in aArray
            if Valtype(a) == 'A'         
                  ...
                  if Valtype(b) == 'A'
                     for EACH c in b
                     	x++                        
        	   	         aValor := Avalia(c)
           	   	      memarea->(dbAppend())
                    		memarea->Id    := alltrim(str(x)) + '.' + alltrim(str(a:__ENumIndex)) + '.' + alltrim(str(b:__ENumIndex)) + '.' + alltrim(str(c:__ENumIndex))
			               memarea->Kt    := ValType(a:__EnumKey)
			               memarea->Key   := Alltrim(hb_ValToStr(a:__EnumKey))
                     ...
                     aValor := Avalia(b)
                     memarea->(dbAppend())
                     memarea->Id    := alltrim(str(x)) + '.' + alltrim(str(a:__ENumIndex)) + '.' + alltrim(str(b:__ENumIndex))
		               memarea->Kt    := ValType(a:__EnumKey)
		               memarea->Key   := Alltrim(hb_ValToStr(a:__EnumKey))
                     memarea->Valor := aValor[1]
                     memarea->Type  := aValor[2]
               ...
               aValor := Avalia(a)               
               memarea->(dbAppend())
               memarea->Id    := alltrim(str(x)) + '.' + alltrim(str(a:__ENumIndex))
					memarea->Kt    := ValType(a:__EnumKey)
			      memarea->Key   := Alltrim(hb_ValToStr(a:__EnumKey))
               memarea->Valor := aValor[1]
					memarea->Type  := aValor[2]
...
function Avalia(var)   
   ...
   otherwise
   endswitch
   return {cStr, nType, nLen, nDec, cName }
